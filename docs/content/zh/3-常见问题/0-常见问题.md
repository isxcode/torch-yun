...
```json
{
  "spark.kubernetes.driver.podTemplateFile":"/home/zhihuiyun/pod-init.yaml",
  "spark.kubernetes.executor.podTemplateFile":"/home/zhihuiyun/pod-init.yaml"
}
```

```bash
vim /home/zhihuiyun/pod-init.yaml
```
...
```text
这是因为spark的standalone集群版本，和torch-yun中spark-min中自带的版本冲突，需要将两边的版本保持一致
```

```bash
cp -r /opt/spark/* /home/ispong/torch-yun/spark-min/
rm /home/ispong/torch-yun/spark-min/conf/spark-env.sh
rm /home/ispong/torch-yun/spark-min/conf/spark-defaults.conf
```
...
```log
Caused by: org.apache.hadoop.ipc.RemoteException(org.apache.hadoop.security.AccessControlException): Permission denied: user=185, access=EXECUTE, inode="/tmp":zhihuiyun:supergroup:drwxrwx---
...
> 查看本地zhihuiyun的uid，将容器的权限设置一致

```bash
sudo cat /etc/group
```

> 获取zhihuiyun的uid是1000
...
> runAsUser: 1000 需要和zhihuiyun的uid一致
