...
onMounted(async () => {
  ...
  if (moduleIntro && document.referrer) {
    // 检查背景图是否已在缓存中
    const img = new Image();
    img.src = 'https://zhishuyun-demo.isxcode.com/tools/open/file/bg-0.jpg';

    if (img.complete && img.naturalWidth > 0) {
      // 如果图片已缓存，立即显示
      moduleIntro.style.backgroundImage = `url("https://zhishuyun-demo.isxcode.com/tools/open/file/bg-0.jpg")`;
...
useHead({
  link: [
    {
      rel: 'preload',
      href: 'https://zhishuyun-demo.isxcode.com/tools/open/file/bg-0.jpg',
      as: 'image',
      type: 'image/jpeg'
    }
  ]
...
onMounted(() => {
  ...
  if (container) {
    ...
    poster.src = 'https://zhishuyun-demo.isxcode.com/tools/open/file/product.jpg';
...
// 主题色
$primary-color: #506bfe;
// 特点1背景色
$module-feat-left-bg-color: white;
// 特点2背景色
$module-feat-right-bg-color: rgb(220 234 254 / 30%);
// 特点高度
...
.main {
  ...
  .module-intro {
    ...
    background-color: rgb(220 234 254 / 30%); /* 添加默认背景色 */
    padding-top: 200px;
    position: relative;
    overflow: hidden;
