...
public interface WorkflowInstanceRepository extends JpaRepository<WorkflowInstanceEntity, String> {
    ...
    @Query(value = "select " + "   new com.isxcode.torch.api.instance.ao.WorkflowInstanceAo(" + "   W.id,"
    ...
    @Query("SELECT new com.isxcode.torch.api.monitor.ao.WorkflowMonitorAo( W.id,W1.name,W.duration,W.execStartDateTime,W.execEndDateTime,W.status,U.username ) from WorkflowInstanceEntity W left join WorkflowEntity W1 on W.flowId = W1.id left join UserEntity U on W.lastModifiedBy = U.id where W1.name like %:keyword% and W.tenantId=:tenantId order by W.lastModifiedDateTime desc")
